...
export const resourceDefinitions = resourceCatalogData.map((resource) => ({
    id: resource.id,
    label: resource.label,
    unit: resource.unit,
    description: resource.description,
}));
export const resourceById = resourceDefinitions.reduce((map, item) => {
    map[item.id] = item;
    return map;
}, {});
export const resourceIds = resourceDefinitions.map((item) => item.id);
export const atomVectors = resourceCatalogData.reduce((map, resource) => {
    map[resource.id] = {
...
export function zeroAtomTotals() {
    return { H: 0, C: 0, O: 0, Si: 0, Fe: 0 };
}
export function computeAtomTotals(inventory) {
    return resourceIds.reduce((totals, resourceId) => {
