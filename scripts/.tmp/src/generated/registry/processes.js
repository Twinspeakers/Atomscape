...
function normalizeResourceAmounts(value) {
    ...
    return Object.fromEntries(entries.map(([resourceId, amount]) => [resourceId, Number(amount)]));
}
export const PROCESS_CATALOG = Object.fromEntries(processCatalogData.map((process) => [
    process.id,
    {
        name: process.name,
        ...
        consume: 'consume' in process
            ? normalizeResourceAmounts(process.consume)
            : undefined,
        produce: 'produce' in process
            ? normalizeResourceAmounts(process.produce)
            : undefined,
