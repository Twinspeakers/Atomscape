import {
  ...
  CREW_AUTO_DRINK_THRESHOLD,
  CREW_AUTO_EAT_THRESHOLD,
  ...
  SIMULATION_TICK_ORDER,
} from '../../../src/domain/spec/gameSpec.js'
import { formatQty } from '../formatters.js'
import { AUTO_GENERATED_HEADER, type GeneratedPage } from '../types.js'

export function buildSystemsPage(): GeneratedPage {
  const content = [
    ...
    ...MARKET_PRODUCTS.map(
      (product) =>
        `- ${product.label} (\`${product.productId}\`) base price: **${formatQty(product.basePrice)} cr**`,
    ),
    '',
  ].join('\n')
